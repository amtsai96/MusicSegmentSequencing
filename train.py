...
def main():
    ...
    plot('acc', 'train', epochs, acc_train, color='red', label='Accurracy')
    plot('loss', 'train', epochs, loss_train, color='green', label='Loss')
    plot('emb_norms', 'train', epochs, emb_train, color='cyan', label='Embedding')
...
def test(test_loader, tnet, criterion, epoch):
    ...
    #plot('acc', 'test', epoch, accs.avg, color='red', label='Accurracy')
...
def plot(var_name, split_name, x, y, color, label='', version='00'):
    ...
    plt.xlabel('Epochs')
    plt.ylabel(var_name)
    if not label == '' : plt.legend()
    directory = "runs/%s/img/"%(args.name)
    if not os.path.exists(directory):os.makedirs(directory)
    plt.savefig(os.path.join(directory, '{}_{}_{}.png'.format(version, var_name, split_name)), bbox_inches='tight', pad_inches=0.0)
    plt.close()
    plt.show()
