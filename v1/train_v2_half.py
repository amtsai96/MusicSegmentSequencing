...
def train(train_loader, tnet, criterion, optimizer, epoch):
    ...
    for batch_idx, (A, P, N) in enumerate(train_loader):
        ...
        # compute output
        dists, embeddings = tnet(A, P, N)
        # 1 means, dist_a-p should be larger than dist_a-n
        target = torch.FloatTensor(dists[0].size()).fill_(1) # Dist_A-P
        ...
        for i in range(1, len(dists)):
            loss_triplet = criterion(dists[0], dists[i], target)
            loss_list.append(loss_triplet.item())
            #total_loss += loss_triplet.item()
            loss_embedd = embeddings[0].norm(2) + embeddings[1].norm(2)
            for ni in embeddings[-1]: loss_embedd += ni.norm(2)
            #loss = loss_triplet + 0.001 * loss_embedd
            loss += loss_triplet
        
            # measure accuracy and record loss
            acc = accuracy(dists[0], dists[i])
        ...
        if batch_idx % args.log_interval == 0:
            print('Train Epoch: {} [{}/{}]\t'
                  ...
                  'Emb_Norm: {:.2f} ({:.2f})'.format(
                      ...
                      losses.val, losses.avg, 100. * accs.val, 100. * accs.avg, emb_norms.val, emb_norms.avg))
